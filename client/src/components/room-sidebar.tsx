...
import { Hash, Settings, User, LogOut, ChevronDown, ChevronRight } from "lucide-react";
...
export default function RoomSidebar({ 
  ...
  const { data: dmRooms } = useQuery<Room[]>({
    queryKey: ["/api/dm", currentUser.id],
    enabled: !!currentUser,
  });

  const { data: allUsersForDM } = useQuery<UserType[]>({
    queryKey: ["/api/users"],
    enabled: !!currentUser,
  });
  ...
  const getDMUserInfo = (room: Room) => {
    ...
    return allUsersForDM.find(user => user.id === otherUserId);
  };
    ...
    <div className="flex flex-col h-full bg-[#1e1f22]/95 backdrop-blur-xl border-r border-white/5 font-sans">
      <div className="p-4 border-b border-white/5 bg-[#2b2d31]/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-black text-white tracking-tight leading-none text-sm">CHAT</h2>
            <h2 className="font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-pink-400 tracking-tight leading-none mt-0.5 text-sm">NEXUS</h2>
          </div>
        </div>
      </div>
